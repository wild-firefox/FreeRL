...
(包含 ma_transformer.py ,transformer_act.py ,util.py) 
'''
...
class MAT: 
    ...
    def all(self):
        """
        从所有智能体的缓冲区中提取数据，并将其转换为张量形式。

        返回:
            obs (Tensor): 观测值张量，形状为 [batch_size, n_agent, obs_dim]
            action (Tensor): 动作张量，形状为 [batch_size, n_agent, action_dim]
            reward (Tensor): 奖励张量，形状为 [batch_size, n_agent, 1]
            next_obs (Tensor): 下一状态的观测值张量，形状为 [batch_size, n_agent, obs_dim]
            done (Tensor): 结束标志张量，形状为 [batch_size, n_agent, 1]
            action_log_pi (Tensor): 动作概率对数张量，形状为 [batch_size, n_agent, action_dim]
            adv_dones (Tensor): 加权结束标志张量，形状为 [batch_size, n_agent, 1]
        """
        # 提取每个智能体的缓冲区数据
        data = [buffer.all() for buffer in self.buffers.values()]
        
        # 使用 zip 函数分别将每种数据类型分组
        obs_list, action_list, reward_list, next_obs_list, done_list, action_log_pi_list, adv_dones_list = zip(*data)
