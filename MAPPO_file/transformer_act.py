import torch  # copy from orginal code
...
def continuous_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
    ...
    for i in range(n_agent):
        ...
        log_std = torch.clamp(decoder.log_std, -20, 2) # exp(-20) - exp(2) 等于 2e-9 - 7.4，确保std在合理范围内
...
def continuous_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv):
    ...
    # print("shifted_action: ", shifted_action.requires_grad)
    # print("obs_rep: ", obs_rep.requires_grad)
    # print("obs: ", obs.requires_grad)
    act_mean = decoder(shifted_action, obs_rep, obs)
    act_mean = torch.tanh(act_mean)
    #print("act_mean: ", act_mean.requires_grad)
    #action_std = torch.sigmoid(decoder.log_std) * 0.5
    log_std = torch.clamp(decoder.log_std, -20, 2) # exp(-20) - exp(2) 等于 2e-9 - 7.4，确保std在合理范围内
    action_std = torch.exp(log_std)

    #print(decoder.log_std)
    distri = Normal(act_mean, action_std)
