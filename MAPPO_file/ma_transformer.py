'''
copy from orginal code
state unused:
尽管这些方法接收了 state 作为参数，但实际并未使用传入的 state，而是将其替换为一个固定的全零张量。
'''
...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs):
        ...
        v_loc = self.head(rep)
        # print("v_loc: ", v_loc.requires_grad)
        # print("rep: ", rep.requires_grad)
...
class Decoder(nn.Module):
    ...
    def forward(self, action, obs_rep, obs):
        ...
        if self.dec_actor:
            ...
            #print("action_embeddings: ", action_embeddings.requires_grad)
            x = self.ln(action_embeddings)
            for block in self.blocks:
                x = block(x, obs_rep)
            logit = self.head(x)
        #print("logit: ", logit.requires_grad)
        ### 个人新增加 使得mean 在-1到1之间
...
class MultiAgentTransformer(nn.Module):
    ...
    def forward(self, state, obs, action, available_actions=None):
        ...
        batch_size = np.shape(obs)[0]  ### !!改成state -> obs!!
        v_loc, obs_rep = self.encoder(state, obs)
        #print("obs_rep: ", obs_rep.requires_grad)
        #print("v_loc: ", v_loc.requires_grad)
        if self.action_type == 'Discrete':
            action = action.long()
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False):
        ...
        v_loc, obs_rep = self.encoder(state, obs)
        #print("obs_rep_get_act: ", obs_rep.requires_grad)
        if self.action_type == "Discrete":
    ...
    def get_values(self, state, obs):
        ...
        state = check(state).to(**self.tpdv)
        obs = check(obs).to(**self.tpdv)
        #print("obs+get: ", obs.requires_grad)
        v_tot, obs_rep = self.encoder(state, obs)
        #print("obs_rep_get: ", obs_rep.requires_grad)
        return v_tot
