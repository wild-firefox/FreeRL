...
class Buffer_for_PPO_mask:
    """replay buffer for each agent"""

    def __init__(self, capacity, obs_dim, act_dim, device, mask_act_dim ,trick=None ):
        self.capacity = capacity = int(capacity)
        ...
        self.available_actions = np.zeros((capacity, mask_act_dim), dtype=bool)  # mask for available actions    
    ...
    def add(self, obs, action, reward, next_obs, done, action_log_probs, adv_done, available_action):
        """ add an experience to the memory """ #
        self.obs[self._index] = obs
        self.actions[self._index] = action
        self.rewards[self._index] = reward
        ...
        self.action_log_probs[self._index] = action_log_probs
        self.adv_dones[self._index] = adv_done
        self.available_actions[self._index] = available_action
    ...
    def all(self):
        ...
        actions_log_probs = torch.as_tensor(self.action_log_probs,dtype=torch.float32).to(self.device) # torch.Size([batch_size, action_dim])
        adv_dones = torch.as_tensor(self.adv_dones,dtype=torch.float32).reshape(-1,1).to(self.device)
        available_actions = torch.as_tensor(self.available_actions, dtype=torch.float32).to(self.device) # # torch.Size([batch_size, mask_act_dim])
        
        return obs, actions, rewards, next_obs, dones , actions_log_probs, adv_dones , available_actions
    
    # 在Buffer.py文件中添加state参数
class Buffer_for_PPO_state_mask(Buffer_for_PPO_mask):
    def __init__(self, horizon, obs_dim, act_dim, device, mask_act_dim,state_dim,):
        super().__init__(horizon, obs_dim, act_dim, device, mask_act_dim)
        self.states = np.zeros((horizon, state_dim))
    
    def add(self, obs, action, reward, next_obs, done, action_log_pi, adv_dones, avail_actions, state):
        # 现有的添加逻辑
        self.states[self._index] = state
        super().add(obs, action, reward, next_obs, done, action_log_pi, adv_dones, avail_actions)
        # 添加状态
        
    
    def all(self):
        states = torch.as_tensor(self.states, dtype=torch.float32).to(self.device)  # torch.Size([batch_size, state_dim])
        obs, action, reward, next_obs, done, action_log_pi, adv_dones, avail_actions = super().all()
        return obs, action, reward, next_obs, done, action_log_pi, adv_dones, avail_actions, states