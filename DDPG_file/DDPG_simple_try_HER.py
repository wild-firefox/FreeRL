...
本代码参考：
https://github.com/ZYunfeii/DRL_algorithm_library/blob/master/DDPG/DDPG_spinningup_HER/main.py
可参考：
https://github.com/openai/baselines/blob/ea25b9e8b234e6ee1bca43083f8f3cf974143998/baselines/her/ddpg.py
之后有时间尝试：
https://blog.csdn.net/hehedadaq/article/details/114005374
'''
...
def generate_goals(i, episode_cache, sample_num=4, sample_range = 200):
    '''
    ...
    if len(epi_to_go) < sample_num:
        ...
        sample_trans = random.sample(epi_to_go, sample_num) # np.random.choice 不能处理包含不同形状的数据
...
if __name__ == '__main__':
    ...
    # trick参数
    parser.add_argument("--policy_name", type=str, default='DDPG_simple_HER')
    ...
    random.seed(args.seed) ## 保证her中的random
    ### cuda
    ...
    ### dim_info
    dim_info = [dim_info[0] + dim_info[0],dim_info[1]] # obs + goal
    ## 算法配置
    ...
    + goal
    '''
    #goal,info = env.reset(seed=args.seed) # 使用初始状态作为目标
    goal = np.array([1, 0, 0], dtype=np.float32)
    goal_dim = dim_info[0]
    obs = np.concatenate((obs, goal))  # concatenate
    ...
    while episode_num < args.max_episodes:
        ...
        next_obs = np.concatenate((next_obs, goal))  # concatenate
        reward_ = calcu_reward(goal,obs,action_)
        episode_cache.append((obs,action_,reward_,next_obs))

        done = terminated or truncated
        done_bool = terminated     ### truncated 为超过最大步数
        policy.add(obs, action, reward_, next_obs, done_bool) # 用新计算的reward_
