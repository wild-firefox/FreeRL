...
2. 使用和SAC类似的方法：重参数化方法(将随机变量的采样与模型参数分离),让离散分布的采样可导  在ddpg中的使用就是 Gumbel-Softmax 技巧 
...
class DDPG: 
    ...
    def select_action(self, obs):
        ...
        if self.is_continue: # dqn 无此项
            ...
            '''
            hard=True : distribution ->one-hot # gumbel_softmax 是有噪声的值, onehot_from_logits 确定值
            '''
            logits = self.agent.actor(obs)
            #action = onehot_from_logits(logits)
            action = gumbel_softmax(logits, hard=True)#.detach().cpu().numpy().squeeze(0) # 1 x action_dim 
            #action = action.argmax() # from one-hot to 真实值
            #action_noise = action_noise.detach().cpu().numpy().squeeze(0)
            action = action.detach().cpu().numpy().squeeze(0)
            #print(action,type(action),type(action.item()))
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        #     # the action from env.action_space.sample() is int, we have to convert it to onehot -> for update critic 1xaction_dim
    ...
    ## 算法相关
    def learn(self,batch_size, gamma, tau, temperature=2.0):
        ...
        if self.is_continue:
            ...
            logits = self.agent.actor_target(next_obs)
            #next_action_ , next_action = gumbel_softmax(logits, hard=True) #onehot_from_logits(logits,eps=0.0)#gumbel_softmax(logits, hard=True)#onehot_from_logits(logits,eps=0.0) # batch_size x action_dim
            next_action = onehot_from_logits(logits,eps=0.0)
        ...
        if self.is_continue:
            ...
            new_logits = self.agent.actor(obs)
            new_action  = gumbel_softmax(new_logits,hard=True,temperature=1)#onehot_from_logits(new_logits,eps=0.0) #gumbel_softmax(new_logits,hard=True)
    ...
    while episode_num < args.max_episodes:
        ...
        if step < args.random_steps:
            ...
            if max_action is not None:
                ...
                action = np.eye(action_dim)[action_]
        else:
            
            if max_action is not None:
                action = policy.select_action(obs)  # (-1,1) or one-hot
        ...
        if args.is_con_to_dis and max_action is None: #如果环境离散，且算法（用于连续）使用连续转离散域技巧
