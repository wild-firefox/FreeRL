...
#### 回去把1 masac的actor中的x改为obs 2.ppo的Agent加入dim_info
'''
一个深度强化学习算法分三个部分实现：
...
'''
class Actor_discrete_hands_on(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_1=128, hidden_2=128):
        super(Actor_discrete_hands_on, self).__init__()
        self.l1 = nn.Linear(obs_dim, hidden_1)
        self.l2 = nn.Linear(hidden_1, hidden_2)
...
class Critic_discrete_hands_on(nn.Module):
    def __init__(self, dim_info, hidden_1=128,hidden_2=128):
        super(Critic_discrete_hands_on, self).__init__()
        ...
        # V2
        self.l4 = nn.Linear(obs_dim, hidden_1)
        self.l5 = nn.Linear(hidden_1, hidden_2)
        self.l6 = nn.Linear(hidden_2, action_dim)

    def forward(self, obs ):
        
        v1 = F.relu(self.l1(obs))
        v1 = F.relu(self.l2(v1))
        v1 = self.l3(v1)

        v2 = F.relu(self.l4(obs))
...
class Agent:
    def __init__(self, obs_dim, action_dim, dim_info ,actor_lr, critic_lr, is_continue, device , discrete_type):
        
        if is_continue:
            self.actor = Actor(obs_dim, action_dim, ).to(device)
            self.critic = Critic( dim_info ).to(device)
        else:
            if discrete_type['hands_on']:
                self.actor = Actor_discrete_hands_on(obs_dim, action_dim, ).to(device)
                self.critic = Critic_discrete_hands_on( dim_info).to(device)
...
class SAC:
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, buffer_size, device, trick = None):
        ...
        ## 离散部分
        is_hands_on_discrete = True
        self.discrete_type = {'hands_on':True,'other':False} if is_hands_on_discrete else {'hands_on':False,'other':True} # other留给其他实现
        print(self.discrete_type) if is_continue is False else None
        ##
        obs_dim, action_dim = dim_info
        self.agent = Agent(obs_dim, action_dim,dim_info,actor_lr, critic_lr, is_continue, device, self.discrete_type)
    ...
    def select_action(self, obs):
        ...
        elif self.discrete_type['hands_on']:
            action_dist = Categorical(probs = self.agent.actor(obs))
    ...
    def learn(self, batch_size ,gamma , tau):
        ...
        if self.is_continue:
            ...
            next_probs = self.agent.actor(next_obs)
            next_log_probs = torch.log(next_probs + 1e-8)
            next_V1_target,next_V2_target = self.agent.critic_target(next_obs)
            next_Q_target = torch.sum(next_probs * torch.min(next_V1_target, next_V2_target),dim=1, keepdim=True)
            '''SAC 特有'''
            entropy_next = -torch.sum(next_probs * next_log_probs, dim=1, keepdim=True)
        ...
        if self.is_continue:
            ...
            current_V1,current_V2 = self.agent.critic(obs)# batch_size x 1
            current_Q1 = current_V1.gather(1, actions.long())
            current_Q2 = current_V2.gather(1, actions.long())
        ...
        if is_continue:
            ...
            new_probs = self.agent.actor(obs)
            new_log_pi = torch.log(new_probs + 1e-8)
            entropy = -torch.sum(new_probs * new_log_pi, dim=1, keepdim=True) # 直接根据概率计算熵
            V1_pi , V2_pi = self.agent.critic(obs)
            Q_pi = torch.sum(new_probs * torch.min(V1_pi, V2_pi),dim=1, keepdim=True)  # 比如这里的hands_on 是用的min 而不是mean
            actor_loss = (- Q_pi - self.alphas.alpha.detach() * entropy).mean()  #这里alpha一定要加detach(),因为在更新critic时,计算图被丢掉了
...
if __name__ == '__main__':
    ...
    # trick参数
    parser.add_argument("--policy_name", type=str, default='SAC_add_discrete') 
    parser.add_argument("--trick", type=dict, default={'Batch_ObsNorm':True}) # 这里不加入ObsNorm 技巧 原因：不如Batch_ObsNorm稳定
    ...
    while episode_num < args.max_episodes:
        ...
        elif policy.discrete_type['hands_on']:
            if step < args.random_steps:
