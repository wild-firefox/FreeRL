...
class Actor(nn.Module):
    ...
    def forward(self, obs, preference):
        ...
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        x = F.tanh(self.l3(x)) # batch_size X (action_dim )
...
class Critic(nn.Module):
    def __init__(self, dim_info:list, reward_dim, hidden_1 = 256 ,hidden_2=256):
        super(Critic, self).__init__()
        self.action_dim = dim_info[1] #action_dim
        self.reward_dim = reward_dim
        obs_act_dim = sum(dim_info)  

        self.l1 = nn.Linear(obs_act_dim + reward_dim, hidden_1)
        self.l2 = nn.Linear(hidden_1, hidden_2)
        self.l3 = nn.Linear(hidden_2, reward_dim)

    def forward(self, obs, action, preference):
        '''对比源代码
        1.减少了两层hidden层
        2.将hq的代码写到buffer的add中，增加理解性
        '''
        ...
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        q = self.l3(x) # batch_size x reward_dim
...
class Agent:
    def __init__(self, obs_dim, action_dim, reward_dim ,actor_lr , critic_lr , device):

        self.actor = Actor(obs_dim, action_dim, reward_dim).to(device)
        self.critic = Critic([obs_dim, action_dim], reward_dim).to(device)
...
## 第二部分：定义DQN算法类
class ENVELOPE_DDPG:
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, buffer_size, device, beta , max_episodes, trick = None):
        obs_dim, action_dim , reward_dim = dim_info #
        self.agent = Agent(obs_dim, action_dim, reward_dim, actor_lr, critic_lr, device)
    ...
    def select_action(self, obs):
        ...
        if self.is_continue:  # dqn 均取离散动作
            action = self.agent.actor(obs,preference).detach().cpu().numpy().squeeze(0) 
        else:
            print('error:ENVELOPE_DDPG中不支持离散动作')
    ...
    def add(self, obs, action, reward, next_obs, done ,gamma ):
        ...
        with torch.no_grad():
            ...
            q = self.agent.critic(torch.as_tensor(obs,dtype=torch.float32).reshape(1, -1).to(self.device),torch.as_tensor(action,dtype=torch.float32).reshape(1, -1).to(self.device),preference.reshape(1,-1)) # 1 x reward_dim
            q = q.squeeze(0) # reward_dim
            wq = preference.dot(q) # 1  # wTQ

            wr = preference.dot(torch.tensor(reward,dtype=torch.float32).to(self.device)) # 1 # wTr 加权之后的奖励
            if not done:
                ...
                next_q = self.agent.critic(torch.as_tensor(next_obs,dtype=torch.float32).reshape(1, -1).to(self.device),self.agent.actor(torch.as_tensor(next_obs,dtype=torch.float32).reshape(1, -1).to(self.device),preference.reshape(1,-1)),preference.reshape(1,-1)) # 1 x reward_dim
                hq = next_q.squeeze(0) # reward_dim
    ...
    def learn(self,batch_size, gamma, tau, weight_num ,update_freq):
        ...
        next_action = self.agent.actor(next_obs,w_batch) # (batch_size x weight_num) x action_dim
        next_Q_target = self.agent.critic_target(next_obs,next_action,w_batch) # (batch_size x weight_num) x reward_dim
        ...
        current_Q = self.agent.critic(obs,actions,w_batch)  # (batch_size x weight_num) x reward_dim
        ...
        self.agent.update_critic(self.loss)

        # 更新actor
        new_actions = self.agent.actor(obs,w_batch) # (batch_size x weight_num) x action_dim
        actor_loss = -self.agent.critic(obs,new_actions,w_batch).mean() # (batch_size x weight_num) x reward_dim -> (batch_size x weight_num) x 1 -> 1
    ...
    def update_target(self, tau):
        ...
        '''
        self.soft_update(self.agent.critic_target, self.agent.critic, tau)
        self.soft_update(self.agent.actor_target, self.agent.actor, tau)
    ...
    def save(self, model_dir):
        torch.save(self.agent.actor.state_dict(), os.path.join(model_dir,"ENVELOPE_DDPG.pt"))
    ## 加载模型
    @staticmethod 
    def load(dim_info, is_continue ,model_dir, trick = None):
        policy = ENVELOPE_DDPG(dim_info,is_continue,0,0,0,device = torch.device("cpu"),trick = trick, beta=0,max_episodes=1)
        policy.agent.actor.load_state_dict(torch.load(os.path.join(model_dir,"ENVELOPE_DDPG.pt")))
        return policy
...
def eval_model(policy, env, w):
    ## 评估一次
    obs_e,info = env.reset()
    episode_reward = 0
    done_e = False
    ...
    while not done_e:
        ...
        action_ = np.clip(action * max_action, -max_action, max_action) #if max_action is not None else action

        next_obs, reward,  = env.step(action_,step) 
        
        done_e = True if step == 59 else False
...
if __name__ == '__main__':
    ...
    # 环境参数
    parser.add_argument("--env_name", type = str,default="env_work") 
    # 共有参数
    ...
    # DDPG 独有参数 noise
    ## gauss noise
    parser.add_argument("--gauss_sigma", type=float, default=0.2) # 高斯标准差 # 0.1 (1 for MountainCarContinuous-v0 )
    ...
    # trick参数
    parser.add_argument("--policy_name", type=str, default='ENVELOPE_DDPG')
    parser.add_argument("--trick", type=dict, default={'reward_norm':True})    
    # device参数
    ...
    env = Environment()
    dim_info = [env.observation_space,env.action_space,env.reward_size]         
    max_action = 0.1
    is_continue = True
    ...
    ## 算法配置
    policy = ENVELOPE_DDPG(dim_info, is_continue, actor_lr = args.actor_lr, critic_lr = args.critic_lr, buffer_size = args.buffer_size,device = device, beta = args.beta, max_episodes = args.max_episodes,)
    ...
    if args.trick['reward_norm']:  
        reward_norm = Normalization(shape=env.reward_size)
    while episode_num < args.max_episodes:
        ...
        # 探索环境
        next_obs, reward, = env.step(action_,episode_step) 
        print(episode_num,episode_step,action_,reward ,next_obs)
        done = True if episode_step == 59 else False #49# max_episode_steps = 100
        done_bool = False
        ...
        if args.trick['reward_norm']:
            policy.add(obs, action, reward_, next_obs, done_bool, args.gamma)
        else:
        ...
        if done:
            ...
            if episode_num % args.evaluate_interval == 0:
                ...
                episode_reward = eval_model(policy, env, w = w_eval )
