...
class PPO:
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, horizon, device, trick = None):
        ...
        self.buffer = Buffer_for_PPO(horizon, obs_dim, act_dim = action_dim if is_continue else 1, device = device,trick = {'decaystd':True}) #Buffer中说明了act_dim和action_dim的区别
        self.device = device
        ...
        ## dacay std # 根据环境修改
        if self.is_continue:
            self.act_std_min=0.05
            self.act_std_init=0.3
            self.act_std = self.act_std_init
    ...
    def select_action(self, obs):
        ...
        if self.is_continue: # dqn 无此项
            ...
            action_log_pi = dist.log_prob(action) # 1
            # to 真实值
    ...
    def learn(self, minibatch_size, gamma, lmbda ,clip_param, K_epochs, entropy_coefficient):
        ...
        for _ in range(K_epochs): 
            ...
            for index in indexes:
                ...
                dist_now = MultivariateNormal(mean, cov_mat)
                ...
                '''
                ratios = torch.exp(action_log_pi_now - action_log_pi[index])  # shape(mini_batch_size X 1)
...
if __name__ == '__main__':
    ...
    while episode_num < args.max_episodes:
        ...
        if done:
            ...
            if is_continue :
                # decay std
                policy.act_std = max((policy.act_std - (policy.act_std_init-policy.act_std_min)/args.max_episodes) , policy.act_std_min)
