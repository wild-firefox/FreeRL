...
import numpy as np
from Buffer import Buffer,PER_Buffer
...
'''
tricks实现:
1.Double DQN:将Q值的选择和评估分开，减少Q值的高估
...
class Agent:
    def __init__(self, obs_dim, action_dim, Qnet_lr , device, trick = None):
        if trick['Dueling']:
            self.Qnet = Dueling(obs_dim, action_dim).to(device)
        else:
...
class DQN:
    def __init__(self, dim_info, is_continue, Qnet_lr, buffer_size, device, trick = None):
        ...
        self.agent = Agent(obs_dim, action_dim, Qnet_lr, device,trick = trick)
        if trick['PER']:
...
def get_env(env_name,dis_to_con_b = False):
    ...
    if isinstance(env.env.observation_space, gym.spaces.Box):
...
if __name__ == '__main__':
    ...
    # trick参数
    parser.add_argument("--trick", type=dict, default={'Double':False,'Dueling':False,'PER':True,'HER':False,'Noisy':False,'n_step':False})  
    ...
    ## 算法配置
    policy = DQN(dim_info, is_continue, Qnet_lr = args.Qnet_lr, buffer_size = args.buffer_size,device = device,trick= args.trick)
